...
```
The mean total number of steps taken per day is `r dayMean1a` and the median is `r dayMedian1a`.
...
```
The 5 minute interval with the maximum average number of steps starts `r intervalWithMaxAvgSteps` minutes into the day.
...
```
There are `r numberOfNAs` missing data values in the set.
...
```
The mean total number of steps taken per day is `r dayMean3a` and the median is `r dayMedian3a`.
